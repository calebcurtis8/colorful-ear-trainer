import { playSequence } from './play_sequence.js'
...
function formatChord (set) {
  return Transposer.transpose({ set, octave: User.get('cadenceoctave', 'number'), flatten: false, withOctave: true })
}
...
const Tonality = function () {
  const progression = User.get('tonality').split(',')
...
const Cadence = () => {
  const progression = Tonality().map((chord, i) => {
    return {
