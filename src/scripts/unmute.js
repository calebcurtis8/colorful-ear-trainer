...
export function unmute (context, allowBackgroundPlayback, forceIOSBehavior) {
  ...
  function updatePlaybackState () {
    ...
    const shouldAllowPlayback = (!!(allowBackgroundPlayback || ( // always be active if noPause is indicated
  ...
  function huffman (count, repeatStr) {
    let e = repeatStr; for (; count > 1; count--) { e += repeatStr } return e
  }
  /**
  ...
  const mediaPlaybackEvents = ['click', 'contextmenu', 'auxclick', 'dblclick', 'mousedown', 'mouseup', 'touchend', 'keydown', 'keyup']
